...
resourcename="dinosaurnurserydb"  # lower case, only
resourcegroup="dinosaurnurserydb_rg"
...
# git clone https://github.com/apilogicserver/dinosaurnurserydb.git
# cd dinosaurnurserydb
...
az group create --name dinosaurnurserydb_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group dinosaurnurserydb_rg --sku S1 --is-linux
az container create --resource-group dinosaurnurserydb_rg --name dinosaurnurserydb --image apilogicserver/dinosaurnurserydb:latest --dns-name-label dinosaurnurserydb --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//dinosaurnurserydb.westus.azurecontainer.io:5656'
